...
def measure_char_width(font: ImageFont.ImageFont) -> int:
    """Advance width of a typical ACARS glyph (monospace 'M')."""
    try:
        bbox = font.getbbox("M")
        w = int(bbox[2] - bbox[0])
        if w > 0:
        ...
        pass
    try:
        w = int(font.getlength("M"))  # type: ignore[attr-defined]
        if w > 0:
            return w
    except Exception:  # noqa: BLE001
        pass
...
def columns_for_bitmap(paper_width: str, glyph_px: int, *, bold: bool = False) -> int:
    ...
    char_w = max(4, measure_char_width(font))
    return max(16, dots // char_w)
